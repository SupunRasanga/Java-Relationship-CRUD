...
public class Services {
	//Create
		public static void createData(DTO dto) throws SQLException {
			...
			String innerSql1 = "INSERT INTO book (book_id,book_name) VALUES(?,?)";
			String innerSql2 = "INSERT INTO author(author_id,author_name,book_id) VALUES(?,?,?)";
			
			try {
				connection = JDBCUtil.getConnection();
				try {
					prest1 = connection.prepareStatement(innerSql1);
					prest1.setLong(1, dto.getBookId());
					prest1.setString(2, dto.getBookName());
				...
				prest2.setLong(1, dto.getAuthorId());
		    	prest2.setString(2, dto.getAuthorName());
		    	prest2.setLong(3, dto.getBookId());
		...
		public static List<DTO> getAllData()throws SQLException{
			...
			String selectSQL = "SELECT book_id,book_name,author_id,author_name FROM author FULL OUTER JOIN book USING(book_id)";
		...
		public static DTO getDataById(long dataId)throws SQLException{
			...
			String getByIdSQL = "SELECT * FROM book FULL OUTER JOIN author USING(book_id) WHERE book_id=? OR author_id=?;";
			DTO dto = new DTO();
			try {
				connection = JDBCUtil.getConnection();
